...
def Extract_Data(browser):
    ...
    try:
        price = clean_price_and_area(
            browser.find_element_by_xpath("//span[preceding-sibling::i[1][contains(@class,'fa-money')]]").text, ',')
    except NoSuchElementException or AttributeError:
        price = ""
    ...
    try:
        built_area = clean_price_and_area(
            browser.find_element_by_xpath("//span[preceding-sibling::i[1][contains(@class,'fa-ruler-triangle')]]").text,
            ',')
    except NoSuchElementException or AttributeError:
        built_area = ""

    try:
        lot_size = clean_price_and_area(
            browser.find_element_by_xpath("//span[preceding-sibling::i[1][contains(@class,'fa-expand')]]").text, ',')
    except NoSuchElementException or AttributeError:
        lot_size = ""
